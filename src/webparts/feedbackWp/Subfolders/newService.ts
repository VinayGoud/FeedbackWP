...
export class FeedbackService {
  ...
  private async fetchAvailableFields(): Promise<Set<string>> {
    ...
    if (!response.ok) {
      // If even checking the schema fails, degrade to "no translation columns
      // available" rather than throwing — the prompt question should still
      // work in English even if this check itself has a problem.
