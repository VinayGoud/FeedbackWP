...
/**
  * SVG icons are defined inline here to avoid the complexity of bundling
 *
 * STEP 2 — import them at the top of this file, alongside the other imports:
 ...
 *
 * STEP 3 — replace each function body below with an <img> tag, e.g.:
 *   function ClipboardIcon(): JSX.Element {
 *     return <img src={clipboardIconUrl} width={20} height={20} alt="" />;
 *   }
 *
 * CAVEAT: the Yes/No icons currently recolor to white when selected via
 * stroke={selected ? '#FFFFFF' : 'currentColor'} on the SVG path — a static
 * image file can't do that automatically. If switching ThumbsUpIcon/
 * ThumbsDownIcon to images, you'll need two separate files per icon
 * (e.g. thumbs-up.svg + thumbs-up-selected.svg) and pick between them:
 *   function ThumbsUpIcon({ selected }: { selected: boolean }): JSX.Element {
 *     return (
 *       <img
 *         src={selected ? thumbsUpSelectedIconUrl : thumbsUpIconUrl}
 *         width={14}
 *         height={14}
 *         alt=""
 *       />
